...
subject_list <- all_data %>%
  mutate(
    ...
    Custom_Sample = case_when(
      ...
      MEGA_Sample != "Nasal" & TOTAL_Nanodrop_ug_nasal >= 1.1 ~ "",
      MEGA_Sample == "Nasal" & TOTAL_Qubit_ug_nasal >= 1.2 ~ "",
      MEGA_Sample == "Nasal" & TOTAL_Nanodrop_ug_nasal >= 1.3 ~ "",
      TRUE ~ "Failed"
    ),
    ...
    Custom_Status_Info = case_when(
      ...
      MEGA_Sample == "Nasal" & TOTAL_Qubit_ug_nasal < 1.2 ~ "Qubit Nasal < 1.2 ug and MEGA sample is Nasal DNA",
      TRUE ~ ""
    )
  )
