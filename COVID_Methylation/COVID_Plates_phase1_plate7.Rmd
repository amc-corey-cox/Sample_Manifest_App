...
files <- c("COVID_phase1_samples_full_manifest_060320_old.xlsx")
...
bsc_info <- read_excel("Manifests/20200527 BSC Plate Manifest_060520.xlsx", sheet = "20200527 BSC", skip = 2) %>%
...
clean_pheno <- raw_pheno %>%
  filter(is.na(`Notes from Biobank`) | str_detect(`Notes from Biobank`, "low|fail", negate = TRUE)) %>%
  mutate( Full_ID = str_c(`Sample Number`, `Plate ID`, sep = "-"),
    `Covid +/-` = factor(`Covid +/-`, levels = c("Positive", "Negative"))
...
sampled_phenos <- clean_pheno %>% nest(data = -`Covid +/-`) %>%
  mutate(n = c(19, 35)) %>% mutate(samp = map2(data, n, sample_n)) %>%
  select(-data) %>% unnest(cols = c(samp)) %>%
  left_join(bsc_info, by = "Full_ID")

recent_plate <- read_excel("Manifests/COVID_Methylation_Manifest_phase1_plate7_test.xlsx") %>%
  mutate(Full_ID = str_c(`Sample ID`, `Plate ID`, sep = "-"))

remaining_phenos <- clean_pheno %>% filter(! Full_ID %in% recent_plate$Full_ID) %>%
  left_join(bsc_info, by = "Full_ID")
...
plates <- reduce(chips, rbind) %>%
  ...
  mutate_at(c("Sample Number", "Unique_Sample_ID"), as.character) %>%
...
write.xlsx(formatted_plates, file = "Manifests/COVID_Methylation_Manifest_phase1_plate7_test.xlsx", showNA = FALSE)
```